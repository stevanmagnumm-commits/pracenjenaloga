...
/**
 * One call per seed. An empty answer is final.
 *
 * Deliberate, and recorded so it is not "fixed" again. This endpoint answers
 * "nothing here" as an OBJECT rather than an empty array —
 *
 *   {"error":"User similar accounts data not found or user profile is private
 *             or does not exist on Instagram"}
 *
 * — which Array.isArray turns into an empty list, and the answer is not stable:
 * probed six seconds apart, @chxrli_love returned 0, then that error, then 78.
 * On a controlled run of 40 seeds, 18 came back empty on the first ask. So
 * asking again would recover real accounts, and it is still not done: the owner
 * wants one call per seed, no second chance.
 */
